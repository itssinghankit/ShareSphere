...
import com.example.sharesphere.data.mapper.toShowCommentsModelList
import com.example.sharesphere.data.mapper.toUserItemModelList
...
class UserRepositoryImplementation @Inject constructor(
    ...
    override suspend fun searchUser(usernameOrName: String): Flow<ApiResult<List<UserItemModel>, DataError.Network>> =
        flow {
            try {
                val response = shareSphereApi.searchUser(usernameOrName).toUserItemModelList()
                emit(ApiResult.Success(response))
            } catch (e: HttpException) {
                when (e.code()) {
                    400 -> emit(ApiResult.Error(DataError.Network.BAD_REQUEST))
    ...
    override suspend fun getFollowers(userId: String): Flow<ApiResult<List<UserItemModel>, DataError.Network>> =
        flow {
            try {
                val response = shareSphereApi.getFollowers(userId).toUserItemModelList()
                emit(ApiResult.Success(response))
            } catch (e: HttpException) {
                when (e.code()) {
                    400 -> emit(ApiResult.Error(DataError.Network.BAD_REQUEST))
    ...
    override suspend fun getFollowing(userId: String): Flow<ApiResult<List<UserItemModel>, DataError.Network>> =
        flow {
            try {
                val response = shareSphereApi.getFollowing(userId).toUserItemModelList()
                emit(ApiResult.Success(response))
            } catch (e: HttpException) {
                when (e.code()) {
                    400 -> emit(ApiResult.Error(DataError.Network.BAD_REQUEST))
    ...
    override suspend fun showComments(postId: String): Flow<ApiResult<List<ShowCommentsModel>, DataError.Network>> =
        flow{
            try {
                val response = shareSphereApi.showComments(postId).toShowCommentsModelList()
                emit(ApiResult.Success(response))
            } catch (e: HttpException) {
                when (e.code()) {
                    400 -> emit(ApiResult.Error(DataError.Network.BAD_REQUEST))
    ...
    override suspend fun addComment(postId: String,content:String): Flow<ApiResult<Unit, DataError.Network>> =
        flow{
            val data=AddCommentReqDto(content)
            try {
                shareSphereApi.addComment(postId,data)
                emit(ApiResult.Success(Unit))
            } catch (e: HttpException) {
                when (e.code()) {
                    400 -> emit(ApiResult.Error(DataError.Network.BAD_REQUEST))
